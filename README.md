...
| `terminal/kitty/` | kitty on the Gruvbox Dark Hard palette (the theme is its own `current-theme.conf`, included), FiraCode Nerd Font Mono with ligatures, an underline caret, copy-on-select, 50k scrollback paged through `bat`, a silent bell, clicks that land — `open-actions.conf` plus a `mime.types` for the extensions Python's table gets wrong — and a `choose-files.conf` so the keyboard file picker agrees with the shell about which files exist |
