...
| Directory         | Holds                                                                                                                                                                                                                                                                                                                                                       |
| ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `shell/`          | zsh in three files split by cost — `zshenv` (every shell), `zprofile` (per login), `zshrc` (interactive). History that searches by real prefix, mise-managed runtimes, fzf + zoxide + eza + atuin (its config under `atuin/`), autosuggestions and syntax highlighting, an `inputrc` for the readline tools, an empty `hushlogin` so a new tab opens at the prompt rather than under the login banner, and `.last-verified`, the author's dated sweep stamp for this surface |
